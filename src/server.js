...
function recipientImportChunkSize() {
...
function buildRecipientInsertChunk(
  registry,
  audienceSnapshotId,
  recipientsChunk
) {
...
async function insertRecipients(client, registry, audienceSnapshotId, recipients) {
  ...
  const chunkSize = recipientImportChunkSize();
  ...
  for (const [index, recipientsChunk] of chunks.entries()) {
    ...
    logger.info("snapshot_import.chunk_completed", {
      ...
      received_recipients: normalized.length,
      duplicate_recipients: Math.max(
        normalized.length - importedCount,
        0
      ),
